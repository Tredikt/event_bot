...
class KeyboardOperations:
    ...
    @staticmethod
    async def __carousel(buttons: list):
        buttons_list = list()
        keyboard = InlineKeyboardBuilder()
        button = InlineKeyboardButton
        for dictionary in buttons:
            for key, value in dictionary.items():
                if isinstance(value, dict):
                    row = list()
                    for text, callback_data in value.items():
                        row.append(button(text=text, callback_data=callback_data))
                    keyboard.row(*row)
                elif isinstance(value, list):
                    for switch_buttons in value:
                        for text, callback_data in switch_buttons.items():
                            keyboard.row(button(text=text, callback_data=callback_data))
                else:
                    keyboard.row(button(text=key, callback_data=value))
            buttons_list.append(keyboard.as_markup())
            keyboard = InlineKeyboardBuilder()
